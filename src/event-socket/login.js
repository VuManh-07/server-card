...
const SinhVien = require('../model/User')
...
const eventLogin = (io, socket) => {
    ...
    io.to(data.roomID).emit('card-response-login-card', {result: data.result, message: data.message});
    })
    ...
    socket.on("card-response-result-sign-message",async (data) => {
        ...
        if(result){
            ...
            let publicKey = ec.recoverPubKey(
                Buffer.from(msgHash, "hex") , signature, signature.recoveryParam, "hex");
            const publicKeyHex = publicKey.encode('hex'); 
            console.log(publicKeyHex);
            const key = ec.keyFromPublic(publicKeyHex, 'hex');
            const isVerified = key.verify(msgHash, signature) 
            if(isVerified){
                await SinhVien.find({publicKey: publicKeyHex})
                .then(data => {
                    ...
                    if(data.length > 0) {
                        ...
                        .then(({data}) => { 
                            ...
                            io.to(roomID).emit("server-responce-login-page", {result: data.result});
                        })
                    }else{
                        io.to(roomID).emit("server-responce-login-page", {result: false, token: "", message: "Không tồn tại user."}); 
                    }
                })
                .catch(err => {
                    console.log(err);
                ...
                io.to(roomID).emit("server-responce-login-page", {result: false, token: "", message: "Please try again."});
            }
        }else{
            io.to(roomID).emit("server-responce-login-page", {result: false, token: "", message: "Please try again."});
        }
